...
!- Implementation Section ----------------------------------
   ...
   do residue = 1, g%num_residues
       ...
       SurfaceOrganicMatterDecomp(residue)%name = g%residue_name(residue)

       SurfaceOrganicMatterDecomp(residue)%OrganicMatterType =g%residue_type(residue)

       !   dsg 131103   The 'amount' value will not be used by SurfaceOrganicMatter, so send zero as default
       SurfaceOrganicMatterDecomp(residue)%amount = 0.0

       SurfaceOrganicMatterDecomp(residue)%C = c_summed
...
!     ===========================================================
subroutine soiln2_ONFreshOrganicMatterIncorporated(variant)
!     ===========================================================
   Use Infrastructure
   implicit none
...
!+  Constant Values
   ...
   parameter (my_name = 'soiln2_ONFreshOrganicMatterIncorporated')
...
!- Implementation Section ----------------------------------
   ...
   do layer = 1, numvals
      ...
      g%fom_c_pool(1,layer) = g%fom_c_pool(1,layer)+ FPoolProfileLayer(layer)%fpool(1)%C
      g%fom_c_pool(2,layer) = g%fom_c_pool(2,layer)+ FPoolProfileLayer(layer)%fpool(2)%C
      g%fom_c_pool(3,layer) = g%fom_c_pool(3,layer)+ FPoolProfileLayer(layer)%fpool(3)%C


      g%fom_n_pool(1,layer) = g%fom_n_pool(1,layer)+ FPoolProfileLayer(layer)%fpool(1)%N
      g%fom_n_pool(2,layer) = g%fom_n_pool(2,layer)+ FPoolProfileLayer(layer)%fpool(2)%N
...
!+  Local Variables
   ...
   real       dlt_fom_n_incorp(max_layer) ! wt of incorporated n material kg/ha
   real       dlt_fom_cnr_incorp(max_layer) ! cnr of incorped material
   real       dlt_fom_c_pool1(max_layer)! FOM C added per layer in fraction 1
   real       dlt_fom_c_pool2(max_layer)! FOM C added per layer in fraction 2
   real       dlt_fom_c_pool3(max_layer)! FOM C added per layer in fraction 3
   real       dlt_fom_n_pool(max_layer)!  FOM N added per layer
   real       dlt_fom_n_pool1(max_layer)! FOM N added per layer in fraction 1
   real       dlt_fom_n_pool2(max_layer)! FOM N added per layer in fraction 2
   real       dlt_fom_n_pool3(max_layer)! FOM N added per layer in fraction 3

   integer    numvals               ! number of values read from file
   integer    numvals1              ! number of values read from file
   integer    numvals2              ! number of values read from file
   integer    numvals3              ! number of values read from file
...
!- Implementation Section ----------------------------------
   ...
   dlt_fom_c_pool1(:) = 0.0
   call collect_real_array_optional ('dlt_fom_c_pool1', max_layer, '(kg/ha)', dlt_fom_c_pool1, numvals1, -100000.0, 100000.0)

   dlt_fom_c_pool2(:) = 0.0
   call collect_real_array_optional ('dlt_fom_c_pool2', max_layer, '(kg/ha)', dlt_fom_c_pool2, numvals2, -100000.0, 100000.0)

   dlt_fom_c_pool3(:) = 0.0
   call collect_real_array_optional ('dlt_fom_c_pool3', max_layer, '(kg/ha)', dlt_fom_c_pool3, numvals3, -100000.0, 100000.0)


   dlt_fom_n_pool1(:) = 0.0
   call collect_real_array_optional ('dlt_fom_n_pool1', max_layer, '(kg/ha)', dlt_fom_n_pool1, numvals_n1, -100000.0, 100000.0)

   dlt_fom_n_pool2(:) = 0.0
   call collect_real_array_optional ('dlt_fom_n_pool2', max_layer, '(kg/ha)', dlt_fom_n_pool2, numvals_n2, -100000.0, 100000.0)

   dlt_fom_n_pool3(:) = 0.0
   call collect_real_array_optional ('dlt_fom_n_pool3', max_layer, '(kg/ha)', dlt_fom_n_pool3, numvals_n3, -100000.0, 100000.0)
...
!     ===========================================================
subroutine soiln2_incorp_min_n ()
!     ===========================================================
   Use Infrastructure
   implicit none

!+  Purpose
!       Add mineral N to soil nh4&no3 pools. The mineral N is due
!       to incorporation via tillage

!+  Mission Statement
!       Add mineral N to nitrogen and ammonium pools

!+  Constant Values
   character  my_name*(*)           ! name of subroutine
   parameter (my_name = 'soiln2_incorp_min_n')

!+  Local Variables
   character  err_string*80         ! Error message string
   integer    layer                 ! layer number in loop ()
!
   real       dlt_nh4_incorp(max_layer) ! nh4 added to top layer(kg/ha)
   real       dlt_no3_incorp(max_layer) ! no3 added to top layer(kg/ha)
   integer    numval_no3                ! number of values read from file
   integer    numval_nh4                ! number of values read from file
   integer    numvals
...
!- Implementation Section ------------------------------
   ...
   call collect_real_array_optional ('dlt_no3', max_layer, '(kg/ha)', dlt_no3_incorp, numval_no3, -10000.0, 10000.0)

   call collect_real_array_optional ('dlt_nh4', max_layer, '(kg/ha)', dlt_nh4_incorp, numval_nh4, -10000.0, 10000.0)


   if (numval_no3 .eq. numval_nh4) then
      ! all ok
      numvals = numval_no3
   else if (numval_no3.eq.0 .or. numval_nh4.eq.0) then
      ! one present - ok?
      numvals = max (numval_no3, numval_nh4)
   else
      ! there is a problem
      err_string = 'depth of incorp is different for NO3 & NH4.'
...
!- Implementation Section ----------------------------------
   ...
   call unpack_SurfaceOrganicMatterDecomp(variant,SurfaceOrganicMatterDecomp,g%num_residues)

   do residue = 1,g%num_residues
      g%residue_name(residue)=SurfaceOrganicMatterDecomp(residue)%name
...
subroutine doInit1()
   ...
   dummy = add_registration_with_units(respondToGetSetReg, 'no3ppm', singleArrayTypeDDML, 'mg/kg')
   ...
   dummy = add_registration_with_units(respondToGetSetReg, 'nh4ppm', singleArrayTypeDDML, 'mg/kg')
   ...
   dummy = add_registration_with_units(respondToSetReg, 'dlt_no3ppm', singleArrayTypeDDML, 'mg/kg')
   dummy = add_registration_with_units(respondToSetReg, 'dlt_nh4ppm', singleArrayTypeDDML, 'mg/kg')
   dummy = add_registration_with_units(respondToSetReg, 'dlt_urea', singleArrayTypeDDML, 'kg/ha')
