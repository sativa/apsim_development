...
c+!!!! perhaps we should get number of layers at init and keep it
     ...
     :                       , dlt_NO3, num_layers)
         ...
         call post_real_array ('dlt_nh4', '(kg/ha)'
     :                       , dlt_Nh4, num_layers)
...
*- Implementation Section ----------------------------------
      ...
      if (variable_name .eq. 'vern_sen') then
...
c      call add_real_array (g%dlt_dm_green, g%dm_green, max_part)
c      call add_real_array (g%dlt_dm_green_retrans, g%dm_green, max_part)
c      call subtract_real_array (g%dlt_dm_senesced, g%dm_green
c     :                        , max_part)

c      call add_real_array (g%dlt_dm_senesced, g%dm_senesced
c     :                   , max_part)
c      call subtract_real_array (g%dlt_dm_sen_retrans, g%dm_green
c     :                        , max_part)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

c      PRINT *, "g%dlt_dm_green(leaf)   = ", g%dlt_dm_green(leaf)
c      PRINT *, "g%dlt_dm_retrans(leaf) = ", g%dlt_dm_green_retrans(leaf)
c      PRINT *, "g%dlt_dm_senesced(leaf)= ", g%dlt_dm_senesced(leaf)
c      PRINT *, "g%dm_green(leaf)       = ", g%dm_green(leaf)
c
c      pause

      do part = 1, max_part

         g%dm_green(part)=g%dm_green(part)+g%dlt_dm_green(part)
     :                                    +g%dlt_dm_green_retrans(part)
     :                                    -g%dlt_dm_senesced(part)

         g%dm_senesced(part)= g%dm_senesced(part)
     .                       +g%dlt_dm_senesced(part)
     .                       -g%dlt_dm_sen_retrans(part)
...
c       PRINT *, "===================================="
c       PRINT *, "g%dlt_tiller_no     =", g%dlt_tiller_no
c       PRINT *, "g%dlt_stiller_no    =", g%dlt_stiller_no
c       PRINT *, "g%tiller_no_fertile =", g%tiller_no_fertile
c       PRINT *, "===================================="
...
c      if ((g%slai.lt.0.0).or.(g%slai.gt.10.0)) then
c      PRINT *, "g%slai              =", g%slai
c      PRINT *, "g%dlt_slai_detached =", g%dlt_slai_detached
c      PRINT *, "dlt_slai_dead       =", dlt_slai_dead
c      PRINT *, "g%dlt_slai          =",g%dlt_slai
c      PRINT *, "g%dlt_slai_age      =",g%dlt_slai_age
c      PRINT *, "g%dlt_slai_light    =",g%dlt_slai_light
c      PRINT *, "g%dlt_slai_water    =",g%dlt_slai_water
c      PRINT *, "g%dlt_slai_nitrogen =",g%dlt_slai_nitrogen
c      pause
c      end if
...
c      call crop_cover1
c     :               (g%row_spacing
c     :                ,c%x_row_spacing,c%y_extinct_coef
c     :                ,c%num_row_spacing
c     :                ,g%lai,g%cover_green)
c      call crop_cover1
c     :               (g%row_spacing
c     :                ,c%x_row_spacing,c%y_extinct_coef_dead
c     :                ,c%num_row_spacing
c     :                ,g%slai,g%cover_sen)
c      call crop_cover1
c     :               (g%row_spacing
c     :                ,c%x_row_spacing,c%y_extinct_coef_dead
c     :                ,c%num_row_spacing
c     :                ,g%tlai_dead,g%cover_dead)
...
c      istage = INT(g%current_stage)

c      PRINT *,"***********************************************"
c      PRINT *, "g%dlt_stage = ", g%dlt_stage
c      PRINT *, "g%leaf_no   = ", g%leaf_no(istage)
c      PRINT *,"***********************************************"
...
*
*   Called by wht_root_incorp in whtsum.for
...
*+  Constant Values
      character  my_name*(*)           ! name of procedure
      parameter (my_name  = 'wht_root_distrib')
...
*     ===========================================================
      implicit none
      include   'action.inc'            ! all_active_modules
...
*
*   Called by wht_end_crop in whtmain.for
*   Calls wht_root_distrib in whtsum.for
...
*+  Constant Values
      character  my_name*(*)           ! name of procedure
      parameter (my_name  = 'wht_root_incorp')
...
*     ===========================================================
      real function Phenology_Stage_Code (
     .          c_stage_code_list,
     .          g_tt_tot,
     .          g_phase_tt,
...
*
*   Called from Crop_N_conc_limits in whtnit.for
...
*+  Constant Values
      character  my_name*(*)           ! name of procedure
      parameter (my_name = 'Phenology_Stage_Code')
...
2000     continue
     ...
     :              , numvals
      ...
      Phenology_Stage_Code = x_stage_code
...
*- Implementation Section ----------------------------------
      ...
      if (stage_is_between (emerg, maturity, g_current_stage)) then
         ...
         current_stage_code = Phenology_Stage_Code (
     .          c_stage_code_list,
     .          g_tt_tot,
     .          g_phase_tt,
     .          g_current_stage,
