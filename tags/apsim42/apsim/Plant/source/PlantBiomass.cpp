 ...
 */
   ...
   if (stage_is_between (start_grnfil, end_grnfil, g_current_stage))
      {

      if (g_dlt_dm_grain_demand > g_dlt_dm_green[grain_part_no])
         {
         ...
         for(int counter = 0 ; counter < num_supply_pools; counter++)
            ...
            dm_part_avail = dm_part_pot -
                             g_dm_plant_min[supply_pools[counter]] * g_plants;
            dm_part_avail = l_bound (dm_part_avail, 0.0);

            dlt_dm_retrans_part = min (dm_grain_differential, dm_part_avail);

            dm_grain_differential = dm_grain_differential - dlt_dm_retrans_part;
            dm_retranslocate[supply_pools[counter]] = -1 * dlt_dm_retrans_part;
            }

         dm_retranslocate[grain_part_no]
                         = -1 * sum_real_array (dm_retranslocate, max_part);
