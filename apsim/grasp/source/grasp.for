...
*- Implementation Section ----------------------------------
      call push_routine (my_name)
                                ...
                                ! find N for soiln
      call grasp_N_uptake (g%N_uptake, g%dlt_No3)
...
*     Limit this to what is available
      do 1000 layer = 1, deepest_layer
         N_avail(layer) = g%No3(layer) - g%No3_min(layer)
 1000 continue
...
!     :                   'Grasp does not work sensibly with soiln.')
      ...
      call fill_real_array (g%No3_min, c%No3_min_lb, max_layer)
      call get_real_array_optional (unknown_module, 'no3_min', max_layer
     :     ,  '(kg/ha)', g%No3_min, numvals, c%No3_min_lb, c%No3_min_ub)
