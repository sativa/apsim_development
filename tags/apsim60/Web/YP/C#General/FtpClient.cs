...
namespace WebCamService
{
	...
	public class FtpClient
	{
		...
		public void UploadDirectory(string path, bool recurse, string mask)
		{
			...
			//dph if ( this.GetFileList(rootDir).Length < 1 ) this.MakeDir(rootDir);
			// dph - added try/catch block below.
			try
				{
		...
		public void RenameFile(string oldFileName,string newFileName, bool overwrite)
		{
			...
			//dphif ( !overwrite && this.GetFileList(newFileName).Length > 0 ) throw new FtpException("File already exists");
