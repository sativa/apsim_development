...
void DatabaseAddIn::askUserForDataSets(void)
   ...
   if (Directory_select_form->ShowModal() == mrOk)
      {
      vector<string> NCFilenames;
      TStrings_2_stl(Directory_select_form->SelectedNCs, NCFilenames);
      readAllNCs(NCFilenames);
      }
   else
      {
...
// ------------------------------------------------------------------
void DatabaseAddIn::readAllNCs(vector<string>& NCFilenames)
   {
   delete_container(simulations);

   for (vector<string>::iterator NC = NCFilenames.begin();
                                 NC != NCFilenames.end();
                                 NC++)
      readNCFile(*NC);
   }
...
// ------------------------------------------------------------------
void DatabaseAddIn::readNCFile(const string& NCFileName)
   {
   // show hourglass.
   TCursor Saved_cursor = Screen->Cursor;
   Screen->Cursor = crHourGlass;

   ApsimNC *NC = new ApsimNC(NCFileName.c_str(), true);

   TStringList *Factors = new TStringList;
   TStringList *Traits = new TStringList;
   TStringList **Levels = new TStringList*[NC->NumFactors()];
   for(int i = 0; i < NC->NumFactors(); i++)
      {
      Levels[i] = new TStringList;
      }

   NC->GetFileAttributes(Factors, Levels, Traits);

   //Get rid of the year factor from the factors TSL
   int YearIndex = FindStringInTSL(Factors, "year");
   Factors->Delete(YearIndex);

   Levels[YearIndex]->Clear();
   //Make a new Levels TSL
   TStringList **NewLevs = new TStringList*[Factors->Count];
   for(int i = 0; i < (Factors->Count); i++)
      {
      NewLevs[i] = new TStringList;
      }
   ...
   for(int j = 0; j < (Factors->Count); j++)
      ...
      if(Levels[j]->Count)
         {
         NewLevs[Count]->Assign(Levels[j]);
         }
      Count++;
      }

   TStringList *SimLevs = new TStringList;
   DBSimulation* newSimulation;

   //Set up counter arrays
   int *current = new int[Factors->Count];
   int *size = new int[Factors->Count];

   //Initialize
   for(int i = 0; i < Factors->Count; i++)
      {
      current[i] = 0;
      size[i] = NewLevs[i]->Count;
      }

   while(current[0] < size[0])
      {
      newSimulation = new DBSimulation();
      SimLevs->Clear();
      for(int i = 0; i < Factors->Count; i++)
         {
         SimLevs->Add(NewLevs[i]->Strings[current[i]]);
         }
      newSimulation->AssignValues(Factors, SimLevs, NC);
      simulations.push_back(newSimulation);


      //Always increment the bottom element of the array
      current[Factors->Count - 1]++;
      //now check if the last string has been read ie. current = size
...
Scenario DatabaseAddIn::getDefaultScenario(void) const
   ...
   if (simulations.size() > 0)
      ...
      for( i = 0; i < simulations.size(); i++)
         {
         if(simulations[i]->IsLegit())
            {
            break;
            }
         }
      return convertSimulationToScenario(*(simulations[i]), "", "");
      }
   else
      {
      vector<Factor> dummy;
...
void DatabaseAddIn::makeScenarioValid(Scenario& scenario, const std::string factorName) const
   ...
   while (f != closestFactorNames.end())
      ...
      if (factorToFind != factorsNamesToFind.end())
         {
         string FacVal = scenario.getFactorValue(*f);
         string titleToTry = titleToFind + ";" + *f + "=" + FacVal;
         ...
         if (closerSimulation != simulations.end() && (*closerSimulation)->IsLegit())
            {
