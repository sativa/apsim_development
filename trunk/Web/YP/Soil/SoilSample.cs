...
namespace CSGeneral
	...
	public class SoilSample : SoilBase
		...
		public double[] CLMapedToSoil
			{
			get 
				{
				double[] soilcl = null;
				if (MyLinkedSoil.CL.Length == 0)
					{
					soilcl = new double[MyLinkedSoil.Thickness.Length];
					for (int i = 0; i != soilcl.Length; i++)
						soilcl[i] = 0;
					}
				else
					{
					soilcl = MyLinkedSoil.CL;
					}
				double[] cl = MapSampleToSoilUsingSpatial(CL, Thickness, soilcl, MyLinkedSoil.Thickness);
				return cl;
				}
			}
