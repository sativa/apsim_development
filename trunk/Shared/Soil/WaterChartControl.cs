...
namespace CSGeneral
	...
	public class WaterChartControl : System.Windows.Forms.UserControl
        ...
        // 
        // 
        this.WaterChart.Axes.Top.Automatic = true;
        // 
        // 
        // 
        ...
        private void WaterChart_ClickLegend(object sender, MouseEventArgs e)
			...
			for (int s = 5; s != WaterChart.Series.Count; s++)
                ...
                if (WaterChart.Series[s].Active)
                    values.Add(MySoil.Crops[s-5]);
                }
            string[] TickedCrops = new string[values.Count];
            values.CopyTo(TickedCrops, 0);
            APSIMSettings.INIWriteMultiple(APSIMSettings.ApsimIniFile(),
										  "soil", "CropsOnGraph", TickedCrops);
			}
