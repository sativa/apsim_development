...
void WheatPhenology::process (const Environment &sw, const pheno_stress_t &ps, float fasw_seed, float pesw_seed)
   ...
   if (inPhase("sowing"))
      ...
      {
      float fstress = min (ps.swdef, ps.nfact);  //eme2fi: (swdef_pheno, g.nfact_pheno);
      if (inPhase("flowering"))
         fstress = ps.swdef_flower;              //swdef_pheno_flower;
      else if (inPhase("start_grain_fill"))
         fstress = ps.swdef_grainfill;           //swdef_pheno_grainfill;
