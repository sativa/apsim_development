...
void genericLeafPart::remove_biomass_update(void)
...
void cohortingLeafPart::doRegistrations(protocol::Component *system)
{
   plantPart::doRegistrations(system);
   setupGetFunction(system, "leaf_no", protocol::DTsingle, false,
                    &cohortingLeafPart::get_leaf_no, "leaves/plant", "Number of leaves per plant");

   system->addGettableVar("node_no", gNodeNo, "nodes/plant", "Number of mainstem nodes per plant");

   setupGetFunction(system, "leaf_no_sen", protocol::DTsingle, false,
                     &cohortingLeafPart::get_leaf_no_sen, "leaves/m2", "Number of senesced leaves per square meter");

   setupGetFunction(system, "leaf_area", protocol::DTsingle, true,
                    &cohortingLeafPart::get_leaf_area, "mm^2", "Leaf area for each node");

   setupGetFunction(system, "leaf_area_tot", protocol::DTsingle, false,
                    &cohortingLeafPart::get_leaf_area_tot, "m^2", "Total plant leaf area");
...
void cohortingLeafPart::get_leaf_no_sen(protocol::Component *system, protocol::QueryValueData &qd)
{
   float sum = 0.0;
   for (int i = 0; i < max_node; i++) sum += gLeafNoSen[i];
   system->sendVariable(qd, sum);
...
//+  Purpose
//     Calculate leaf number development
void cohortingLeafPart::leaf_no_pot (int option, float stressFactor, float dlt_tt)
    {
    if (option == 1)
        {
        cproc_leaf_no_pot1(cNodeAppRate
...
//  on an individual leaf basis, with account taken of the area of
//  currently expanding leaves (node_no_correction).
void cohortingLeafPart::leaf_area_potential ()
   {
   float node_no_now = gNodeNo + cNodeNoCorrection;
